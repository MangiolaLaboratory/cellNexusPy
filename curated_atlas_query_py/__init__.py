...
def get_anndata(
    ...
    files_to_read: pd.Series[str] = (
        data.project("file_id_db").distinct().fetchdf()["file_id_db"].str.cat([".h5ad"])
    )
    ...
    synced = sync_assay_files(
        url=repository, cache_dir=cache_directory, subdirs=subdirs, files=files_to_read
    )
    ...
    for assay_name, files in itertools.groupby(synced, key=lambda x: x[0]):
        ...
        ads = (ad.read_h5ad(path, backed="r")[cells, features] for path in paths)
        ad_per_assay[assay_name] = ad.concat(ads, index_unique="-")

    # Start with the main assay
    main_assay_key = (
        "original" if "original" in ad_per_assay else next(iter(ad_per_assay.keys()))
    )
    ...
    # Attach the observation metadata (which is just the entire metadata table)
